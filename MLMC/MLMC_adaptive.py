"""
Multi-Level Monte Carlo (MLMC) Estimator with Adaptive Sample Allocation
==========================================================================

Implements the full MLMC algorithm with optimal sample allocation across levels
to achieve a target accuracy epsilon with minimal computational cost.
...
Algorithm:
    1. Start with pilot samples (N0 = 100) at L0 = 3 levels
    2. Estimate variance V_l and cost C_l at each level
    3. Compute optimal sample allocation N_l^opt
    4. Run additional samples dN_l = N_l^opt - N_l
    5. Check if remaining bias exceeds eps/sqrt(2)
    6. If yes, add another level and repeat
    7. If no and all N_l are optimal, done!

Physics Analogy:
    Like your Higgs SMEFT analysis, we allocate "statistics budget" optimally:
        - Coarse levels (cheap, high variance): Use many samples
        - Fine levels (expensive, low variance): Use few samples
    The algorithm balances statistical vs systematic errors.
"""
...
from Normal_MC import BS_call, run_mc_estimation
...
def optimal_sample_allocation(V, C, eps):
    ...
    Notes
    -----
    This formula balances variance reduction (need more samples where V_l is high)
    against computational cost (avoid expensive fine levels if possible).
    
    Physics analogy: Like optimising your detector trigger rates. You want high
    statistics where signal/background is unclear, but can afford lower rates
    where discrimination is easy.
    """
...
def mlmc_level_simulation(l, N_l, T, S0, K, r, sigma, h0):
    """
    Run N_l Monte Carlo simulations at level l.
    
    Simulates both fine path (timestep h_l) and coarse path (timestep 2*h_l)
    using coupled Brownian increments for variance reduction.
    
    Parameters
    ----------
    l : int
        MLMC level (determines timestep h_l = h0 * 2^(-l))
    N_l : int
        Number of samples to run at this level
    T : float
        Time to maturity
    S0 : float
        Initial stock price
    ...
    sigma : float
        Volatility
    h0 : float
        Coarsest timestep (at level 0)
        
    Returns
    -------
    sum_Y : float
        Sum of corrections Y = disc*(Pf - Pc) over all samples
    sum_Y_sq : float
        Sum of Y^2 (for variance estimation)
        
    Notes
    -----
    The key MLMC trick: fine and coarse paths use the SAME random numbers,
    creating correlation that reduces Var[Y] dramatically.
    """
    ...
    # Run N_l independent simulations
    for n in range(N_l):
        # ====================================================================
        # Fine Path (timestep h_fine)
        # ====================================================================
        dW_fine = np.random.normal(loc=0.0, scale=math.sqrt(h_fine), size=n_steps)
        
        # Euler-Maruyama: S_{n+1} = S_n + r*S_n*h + sigma*S_n*dW_n
        S = S0
        for dW in dW_fine:
        ...
        # ====================================================================
        # Coarse Path (timestep h_coarse)
        # ====================================================================
        if l > 0:
            # Couple Brownian increments: sum consecutive pairs
...
# Run actual standard MC estimation for comparison
print("Running standard MC estimation for comparison...")
mc_results = run_mc_estimation(
    epsilon_values, T=T, S0=S0, K=K, r=r, sigma=sigma, N0=100000, verbose=False
)
print("Done.\n")
...
print(f"\n{'='*70}")
print("ADAPTIVE MLMC: Optimal Sample Allocation")
print(f"{'='*70}")
...
for eps in epsilon_values:
    ...
    while np.any(dN > 0):
        ...
        for l in range(L + 1):
            if dN[l] > 0:
                # Run dN[l] new samples at level l
                sum_Y, sum_Y_sq = mlmc_level_simulation(
                    l, dN[l], T, S0, K, r, sigma, h0
                )
        ...
        if converged:
            ...
            print(f"  Bias threshold (eps/√2): {eps/math.sqrt(2):.6f}")
            ...
            if bias_estimate > eps / math.sqrt(2):
                ...
                print(f"  ✓ Bias is acceptable, converged!")
                break
    ...
    eps_idx = np.where(epsilon_values == eps)[0][0]
    cost_mc_actual = mc_results['total_cost'][eps_idx]
    ...
    print(f"Normalised cost MLMC (ε²·C): {total_cost:.2f}")
    print(f"Normalised cost MC (ε²·C):   {cost_mc_actual:.2f}")
    print(f"{'='*70}")

    # Record time for this epsilon
    eps_elapsed_time = time.time() - eps_start_time
    print(f"\nTIMING: ε = {eps} converged in {eps_elapsed_time:.2f} seconds")
...
ax1.set_title("Optimal Sample Allocation", fontsize=13, fontweight='bold')

# ========================================================================
# Panel 2: ε² × Cost vs ε (Complexity Analysis)
# ========================================================================
# If cost ~ ε^(-p), then ε² × cost ~ ε^(2-p)
# For optimal MLMC: p = 2, so plot should be flat
# For standard MC: p = 3, so plot grows like ε^(-1)

ax2.loglog(epsilon_values, results_cost,
           marker='o', linestyle='--', linewidth=2, markersize=8,
           color='orange', label='MLMC')

ax2.loglog(epsilon_values, results_cost_mc,
           marker='d', linestyle='--', linewidth=2, markersize=8,
           color='green', label='Standard MC (actual)')
...
plt.tight_layout()
plt.savefig('MLMC_adaptive.pdf',  bbox_inches='tight')
print("✓ Complexity plots saved to: MLMC_adaptive.pdf")
...
print(f"\n{'='*70}")
print("MLMC COMPLEXITY ANALYSIS SUMMARY")
print(f"{'='*70}")
...
for i, eps in enumerate(epsilon_values):
    ...
    print(f"{eps:<12.3f} {results_cost[i]:<15.2f} {results_cost_mc[i]:<15.2f} {speedup:<10.1f}×")
print(f"{'='*70}")
print(f"\nKey insight: MLMC achieves O(ε⁻²) complexity vs O(ε⁻³) for standard MC")
