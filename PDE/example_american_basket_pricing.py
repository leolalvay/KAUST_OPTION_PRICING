...
"""
Example: American Basket Option Pricing with MLMC and Markovian Projection
...
print("\n" + "="*70)
print("AMERICAN BASKET OPTION PRICING WITH MLMC + MARKOVIAN PROJECTION")
print("="*70 + "\n")

# Basket parameters
d = 3  # Number of assets
S0 = np.linspace(225, 275, num=d)[:, np.newaxis]  # Initial prices
basket_weights = np.ones(d) / d  # Equal-weighted basket
...
print(f"  Assets: {d}")
print(f"  Initial prices: {S0.flatten()}")
...
for t_val in [0.0, T/2, T]:
    for S_val in [230, 250, 270]:
        ...
        print(f"  b(t={t_val:.1f}, S={S_val}) = {b_val:.4f}")
...
S_idx_itm = np.argmin(np.abs(S_grid - 0.9*K))
S_idx_otm = np.argmin(np.abs(S_grid - 1.1*K))

print(f"\nOption Values at t=0:")
print(f"  At-the-money (S≈{S_grid[S_idx_atm]:.2f}): {U[0, S_idx_atm]:.4f}")
print(f"  In-the-money (S≈{S_grid[S_idx_itm]:.2f}): {U[0, S_idx_itm]:.4f}")
print(f"  Out-of-the-money (S≈{S_grid[S_idx_otm]:.2f}): {U[0, S_idx_otm]:.4f}")

print(f"\nIntrinsic Values:")
print(f"  At-the-money: {payoff_grid[S_idx_atm]:.4f}")
print(f"  In-the-money: {payoff_grid[S_idx_itm]:.4f}")
print(f"  Out-of-the-money: {payoff_grid[S_idx_otm]:.4f}")
...
print(f"\n{'='*70}")
print(f"⏱️  NUMPY VERSION TOTAL EXECUTION TIME: {elapsed_time:.4f} seconds")
print(f"{'='*70}\n")
...
surf1 = ax1.plot_surface(T_mesh, S_mesh, U, cmap='viridis', alpha=0.9)
...
surf2 = ax2.plot_surface(T_mesh, S_mesh, b_grid, cmap='plasma', alpha=0.9)
...
ax3.plot(S_grid, U[0, :], 'b-', linewidth=2, label='American value')
ax3.plot(S_grid, payoff_grid, 'r--', linewidth=2, label='Intrinsic value')
...
ax5.plot(S_grid, time_value, 'g-', linewidth=2)
...
ax6.plot(S_grid, b_grid[0, :], 'purple', linewidth=2)
...
print(f"\nKey Results:")
print(f"  Option value at S={S_grid[S_idx_atm]:.2f}: {U[0, S_idx_atm]:.4f}")
print(f"  Intrinsic value: {payoff_grid[S_idx_atm]:.4f}")
print(f"  Time value: {U[0, S_idx_atm] - payoff_grid[S_idx_atm]:.4f}")
print(f"  Early exercise premium: {(U[0, S_idx_atm] / payoff_grid[S_idx_atm] - 1)*100:.2f}%")

print(f"\nVolatility Statistics:")
print(f"  Mean volatility: {b_grid.mean():.4f}")
print(f"  Min volatility: {b_grid.min():.4f}")
print(f"  Max volatility: {b_grid.max():.4f}")
