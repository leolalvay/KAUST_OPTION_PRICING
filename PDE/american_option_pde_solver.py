...
"""
American Option PDE Solver using Backward Euler
...
def solve_american_option(
    ...
    Returns
    -------
    t_grid : np.ndarray, shape (N_timesteps + 1,)
        Time grid points from 0 to T
    S_grid : np.ndarray, shape (N_spatial,)
        Spatial grid points from S_min to S_max
    U : np.ndarray, shape (N_timesteps + 1, N_spatial)
        Option value function U(t, S) on grid
    b_grid : np.ndarray, shape (N_timesteps + 1, N_spatial)
        Volatility surface evaluated on grid
    payoff_grid : np.ndarray, shape (N_spatial,)
    ...
    if np.any(b_grid < 0):
        print(f"⚠️  Warning: Negative volatilities detected (min: {b_grid.min():.4f})")
    if np.any(b_grid > 2.0):
        print(f"⚠️  Warning: Extremely high volatilities detected (max: {b_grid.max():.4f})")
    ...
    U[:, 0] = payoff_grid[0]    # At S_min
    U[:, -1] = payoff_grid[-1]  # At S_max
...
def compute_exercise_boundary(t_grid, S_grid, U, payoff_grid, absolute_threshold=0.5):
    ...
    for n, t in enumerate(t_grid):
        ...
        S_before = S_grid[cross_idx - 1]
        S_after = S_grid[cross_idx]
